...
class TLClassifierSimple(object):
    ...
    def crop_box(self, image, box):
        ...
        # return image[int(left), int(bottom), int(right), int(top)]
    ...
    def determine_active_light(self, thresh_img, red_circ, green_circ):
    ...
    def get_classification(self, image):
        ...
        if boxes.size > 0:
            ...
            image = self.crop_box(image, box_coords[max_score_idx])

            # img_np = np.asarray(image)
            circles = self.detect_tl_circles(image)
            red_circ, _, green_circ = self.sort_circles_by_y(circles)

            if red_circ is not None and green_circ is not None:
                thresh_image = self.apply_color_threshold(image)
                red, green = self.determine_active_light(thresh_image, red_circ, green_circ)

                if red and not green:
                    print("RED")
                    return TrafficLight.RED
                elif not red and green:
                    print("GREEN")
                    return TrafficLight.GREEN
                else:
                    print("UNKNOWN")
                    return TrafficLight.UNKNOWN
        # print("UNKNOWN - NOBOXES")
        return TrafficLight.UNKNOWN